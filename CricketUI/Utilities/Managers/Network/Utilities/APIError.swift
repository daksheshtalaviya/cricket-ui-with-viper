...
/// `APIError` is the error type returned. It encompasses a few different types of errors, each with
...
public enum APIError: Error {
    ...
    /// embedded within an `APIError.multipartEncodingFailed` `.inputStreamReadFailed` case.
...
extension Error {
    ...
    /// Returns the instance cast as an `APIError`. If casting fails, a `fatalError` with the specified `message` is thrown.
    ...
    /// Casts the instance as `APIError` or returns `defaultAPIError`
    func asAPIError(or defaultAPIError: @autoclosure () -> APIError) -> APIError {
        self as? APIError ?? defaultAPIError()
    }
}
...
extension APIError.ParameterEncoderFailureReason {
    var underlyingError: Error? {
        switch self {
...
extension APIError.MultipartEncodingFailureReason {
    var url: URL? {
        switch self {
...
#if !(os(Linux) || os(Windows))
extension APIError.ServerTrustFailureReason {
    var output: APIError.ServerTrustFailureReason.Output? {
        switch self {
...
extension APIError.URLRequestValidationFailureReason {
    var localizedDescription: String {
        switch self {
