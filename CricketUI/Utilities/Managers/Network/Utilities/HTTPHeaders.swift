...
extension HTTPHeader {
    ...
    ///
    /// API offers a default Accept-Language header that accumulates and encodes the system's preferred languages.
    ...
    ///
    /// API offers a default accept encoding value that provides the most common values. Use
    ...
    ///
    /// API provides built-in methods to produce `Authorization` headers. For a Basic `Authorization` header use
    ...
    ///
    /// All API `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not
...
extension HTTPHeaders {
    /// The default set of `HTTPHeaders` used by API. Includes `Accept-Encoding`, `Accept-Language`, and
    /// `User-Agent`.
...
extension HTTPHeader {
    /// Returns API's default `Accept-Encoding` header, appropriate for the encodings supported by particular OS
    /// versions.
    ///
    ...
    /// Returns API's default `Accept-Language` header, generated by querying `Locale` for the user's
    /// `preferredLanguages`.
    ///
    ...
    /// Returns API's default `User-Agent` header.
    ///
    /// See the [User-Agent header documentation](https://tools.ietf.org/html/rfc7231#section-5.5.3).
    ///
    /// Example: `iOS Example/1.0 (org.app.iOS-Example; build:1; iOS 13.0.0) API/5.0.0`
    public static let defaultUserAgent: HTTPHeader = {
        ...
        let userAgent = "\(executable)/\(appVersion) (\(bundle); build:\(appBuild); \(osNameVersion)) \(apiVersion)"
