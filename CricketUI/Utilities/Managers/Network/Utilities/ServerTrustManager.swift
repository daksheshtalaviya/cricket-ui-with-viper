//
//  APIServerTrustEvaluation.swift
//  CricketUI
//
...
open class ServerTrustManager {
    ...
    /// - Throws:         `APIError.serverTrustEvaluationFailed` if `allHostsMustBeEvaluated` is `true` and no matching
    ///                   evaluators are found.
    open func serverTrustEvaluator(forHost host: String) throws -> ServerTrustEvaluating? {
        guard let evaluator = evaluators[host] else {
            if allHostsMustBeEvaluated {
                throw APIError.serverTrustEvaluationFailed(reason: .noRequiredEvaluator(host: host))
            }
...
public final class DefaultTrustEvaluator: ServerTrustEvaluating {
    ...
    public func evaluate(_ trust: SecTrust, forHost host: String) throws {
        if validateHost {
            try trust.ax.performValidation(forHost: host)
        }

        try trust.ax.performDefaultValidation(forHost: host)
    }
}
...
public final class RevocationTrustEvaluator: ServerTrustEvaluating {
    ...
    public func evaluate(_ trust: SecTrust, forHost host: String) throws {
        ...
        try trust.ax.validate(policy: SecPolicy.ax.revocation(options: options)) { status, result in
            APIError.serverTrustEvaluationFailed(reason: .revocationCheckFailed(output: .init(host, trust, status, result), options: options))
        }
    }
}
...
public final class PinnedCertificatesTrustEvaluator: ServerTrustEvaluating {
    ...
    ///                                   to performing the default evaluation, even if `performDefaultValidation` is
    ///                                   `false`. `true` by default.
    public init(certificates: [SecCertificate] = Bundle.main.ax.certificates,
    ...
    public func evaluate(_ trust: SecTrust, forHost host: String) throws {
        guard !certificates.isEmpty else {
            throw APIError.serverTrustEvaluationFailed(reason: .noCertificatesFound)
        }

        if acceptSelfSignedCertificates {
            try trust.ax.setAnchorCertificates(certificates)
        }
        ...
        let serverCertificatesData = Set(trust.ax.certificateData)
        let pinnedCertificatesData = Set(certificates.ax.data)
        let pinnedCertificatesInServerData = !serverCertificatesData.isDisjoint(with: pinnedCertificatesData)
        if !pinnedCertificatesInServerData {
            throw APIError.serverTrustEvaluationFailed(reason: .certificatePinningFailed(host: host,
                                                                                        trust: trust,
                                                                                        pinnedCertificates: certificates,
                                                                                        serverCertificates: trust.ax.certificates))
        }
    }
}
...
public final class PublicKeysTrustEvaluator: ServerTrustEvaluating {
    ...
    ///                               `true` by default.
    public init(keys: [SecKey] = Bundle.main.ax.publicKeys,
                performDefaultValidation: Bool = true,
                validateHost: Bool = true) {
        self.keys = keys
        self.performDefaultValidation = performDefaultValidation
    ...
    public func evaluate(_ trust: SecTrust, forHost host: String) throws {
        guard !keys.isEmpty else {
            throw APIError.serverTrustEvaluationFailed(reason: .noPublicKeysFound)
        }
        ...
        let pinnedKeysInServerKeys: Bool = {
            for serverPublicKey in trust.ax.publicKeys {
        ...
        if !pinnedKeysInServerKeys {
            throw APIError.serverTrustEvaluationFailed(reason: .publicKeyPinningFailed(host: host,
                                                                                      trust: trust,
                                                                                      pinnedKeys: keys,
                                                                                      serverKeys: trust.ax.publicKeys))
        }
    }
}
...
extension Bundle: APIExtended {}
public extension APIExtension where ExtendedType: Bundle {
    ...
    var publicKeys: [SecKey] {
        return certificates.ax.publicKeys
    }
...
extension SecTrust: APIExtended {}
public extension APIExtension where ExtendedType == SecTrust {
    ...
    func validate(policy: SecPolicy, errorProducer: (_ status: OSStatus, _ result: SecTrustResultType) -> Error) throws {
        try apply(policy: policy).ax.validate(errorProducer: errorProducer)
    }
    ...
    /// - Throws: An `APIError.serverTrustEvaluationFailed` instance with a `.policyApplicationFailed` reason.
    func apply(policy: SecPolicy) throws -> SecTrust {
        let status = SecTrustSetPolicies(type, policy)

        guard status.ax.isSuccess else {
            throw APIError.serverTrustEvaluationFailed(reason: .policyApplicationFailed(trust: type,
                                                                                       policy: policy,
                                                                                       status: status))
        }
    ...
    func validate(errorProducer: (_ status: OSStatus, _ result: SecTrustResultType) -> Error) throws {
        ...
        guard status.ax.isSuccess && result.ax.isSuccess else {
    ...
    func setAnchorCertificates(_ certificates: [SecCertificate]) throws {
        ...
        guard status.ax.isSuccess else {
            throw APIError.serverTrustEvaluationFailed(reason: .settingAnchorCertificatesFailed(status: status,
                                                                                               certificates: certificates))
        }

        // Reenable system anchor certificates.
        let systemStatus = SecTrustSetAnchorCertificatesOnly(type, true)
        guard systemStatus.ax.isSuccess else {
            throw APIError.serverTrustEvaluationFailed(reason: .settingAnchorCertificatesFailed(status: systemStatus,
                                                                                               certificates: certificates))
        }
    }
    ...
    var certificateData: [Data] {
        return certificates.ax.data
    }

    /// Validates `self` after applying `SecPolicy.ax.default`. This evaluation does not validate the hostname.
    ///
    /// - Parameter host: The hostname, used only in the error output if validation fails.
    /// - Throws: An `APIError.serverTrustEvaluationFailed` instance with a `.defaultEvaluationFailed` reason.
    func performDefaultValidation(forHost host: String) throws {
        try validate(policy: SecPolicy.ax.default) { status, result in
            APIError.serverTrustEvaluationFailed(reason: .defaultEvaluationFailed(output: .init(host, type, status, result)))
        }
    }

    /// Validates `self` after applying `SecPolicy.ax.hostname(host)`, which performs the default validation as well as
    /// hostname validation.
    ///
    /// - Parameter host: The hostname to use in the validation.
    /// - Throws:         An `APIError.serverTrustEvaluationFailed` instance with a `.defaultEvaluationFailed` reason.
    func performValidation(forHost host: String) throws {
        try validate(policy: SecPolicy.ax.hostname(host)) { status, result in
            APIError.serverTrustEvaluationFailed(reason: .hostValidationFailed(output: .init(host, type, status, result)))
        }
    }
}

extension SecPolicy: APIExtended {}
public extension APIExtension where ExtendedType == SecPolicy {
    ...
    ///
    /// - Returns:           The `SecPolicy`.
    /// - Throws:            An `APIError.serverTrustEvaluationFailed` error with reason `.revocationPolicyCreationFailed`
    ...
    static func revocation(options: RevocationTrustEvaluator.Options) throws -> SecPolicy {
        guard let policy = SecPolicyCreateRevocation(options.rawValue) else {
            throw APIError.serverTrustEvaluationFailed(reason: .revocationPolicyCreationFailed)
        }
...
extension Array: APIExtended {}
public extension APIExtension where ExtendedType == [SecCertificate] {
    ...
    var publicKeys: [SecKey] {
        return type.compactMap { $0.ax.publicKey }
    }
}

extension SecCertificate: APIExtended {}
public extension APIExtension where ExtendedType == SecCertificate {
...
extension OSStatus: APIExtended {}
public extension APIExtension where ExtendedType == OSStatus {
    /// Returns whether `self` is `errSecSuccess`.
    var isSuccess: Bool { return type == errSecSuccess }
}

extension SecTrustResultType: APIExtended {}
public extension APIExtension where ExtendedType == SecTrustResultType {
